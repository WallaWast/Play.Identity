...
public class DebitGilConsumer : IConsumer<DebitGil>
{
    ...
    public async Task Consume(ConsumeContext<DebitGil> context)
    {
        ...
        _logger.LogInformation("Received request to debit {Gil} from the user {UserId} with the correlationId {CorrelationId}",
        message.Gil,
        message.UserId,
        ...
        if (user.Gil < 0)
        {
            _logger.LogError("Failed to debit {Gil} from the user {UserId} with the correlationId {CorrelationId}",
                    message.Gil,
                    message.UserId,
                    message.CorrelationId);
